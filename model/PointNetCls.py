...
class STN3d(nn.Module):
    ...
    def forward(self, x):
        ...
        iden = torch.from_numpy(np.array([1, 0, 0, 0, 1, 0, 0, 0, 1]).astype(np.float32)).view(1, 9).repeat(batchsize,
                                                                                                            1)
        if x.is_cuda:
            iden = iden.cuda()
        x = x + iden
...
class STNkd(nn.Module):
    ...
    def forward(self, x):
        ...
        iden = torch.from_numpy(np.eye(self.k).flatten().astype(np.float32)).view(1, self.k * self.k).repeat(batchsize,
                                                                                                             1)
        if x.is_cuda:
            iden = iden.cuda()
        x = x + iden
...
class PointNetCls(nn.Module):
    def __init__(self, feature_transform=False, sample_transform=True, kernel_size=1, stride=1, in_channel=3, dim=3,
                 ext=False, drop=0.3, heinit=False, cvinit=False):
