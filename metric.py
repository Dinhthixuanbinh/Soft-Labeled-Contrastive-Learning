...
def metrics(img_gt, img_pred, pat_id, modality, apply_hd=False, apply_asd=False, class_name=("myo", "lv", "rv"),
    ...
    for c, cls_name in zip(np.array(range(len(class_name))) + 1, class_name):
        ...
        if apply_hd or apply_asd:
            if np.sum(gt_c_i) == 0:
                print("Label empty for Pat {}".format(pat_id))
                size = gt_c_i.shape
                if ndim == 2:
                    gt_c_i[size[0] // 2, size[1] // 2] = 1
                else:
                    gt_c_i[size[0] // 2, size[1] // 2, size[2] // 2] = 1
            # else:
            if np.sum(pred_c_i) == 0:
                print('Prediction empty for Pat {}'.format(pat_id))
                size = pred_c_i.shape
                if ndim == 2:
                    pred_c_i[size[0] // 2, size[1] // 2] = 1
                else:
                    pred_c_i[size[0]//2, size[1]//2, size[2]//2] = 1
            # print("Prediction empty for {} {}".format(modality, pat_id))
            if ifhd95:
                h_d = hd95(gt_c_i, pred_c_i, voxelspacing=spacing)
            else:
                h_d = hd(gt_c_i, pred_c_i, voxelspacing=spacing)
            if apply_asd:
                a_sd = assd(gt_c_i, pred_c_i, voxelspacing=spacing)
