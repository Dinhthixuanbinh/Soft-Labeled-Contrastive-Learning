...
class bSSFPDataSet(data.Dataset):
    def __init__(self, list_path, max_iters=None, crop_size=224, length=None, aug=True):
        self._length = length
        self._aug = aug
...
def get_bssfp_dataloader(scratch, args, input_size_source, length=None, aug=True, shuffle=False):
    bssfp_dataset = bSSFPDataSet(scratch if args.scratch else args.data_dir, max_iters=None,
                                 crop_size=input_size_source, length=length, aug=aug)
    trainloader = data.DataLoader(bssfp_dataset, batch_size=args.batch_size, shuffle=shuffle,
